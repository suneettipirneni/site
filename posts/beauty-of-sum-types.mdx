...
So why do we call this a sum type? It's called a sum type because the type is the sum of all of its variants. In this case, the type is the sum of `Dog` and `Cat`. Notice also how this enum is closed. We can't add any more variants to it. This allows `HousePet` to be exhaustively matched on.
